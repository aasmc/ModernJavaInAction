...
public class SubsetsMain {
    ...
    public static <T> List<List<T>> subsets(List<T> l) {
        ...
        // recursively find all subsets of the sublist without the first element
        List<List<T>> subans = subsets(rest);
        // prefix each list from the subans with the first element taken from the
        // original list.
